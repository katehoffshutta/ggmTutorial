...
A pairwise comparison of coefficients shows that, among coefficients that are nonzero in at least one of the pair of models, coefficients with larger magnitude are typically shrunk less by the SCAD and the MCP penalties but the same phenomenon is not observed in the adaptive graphical lasso: 
...
This can likely be attributed to the very sparse graph that is selected by the adaptive graphical lasso with $\lambda = 0.322$. If we rerun the adaptive graphical lasso with a smaller tuning parameter, we see the expected pattern:
...
par(mfrow=c(1,1),mar=c(2,4,2,4),cex=0.6)
...
plot(allWeights[-rmIndices,1],pch=20,col="red",ylab = "Edge Weight", main = "glasso edge weights vs.dense adaptive glasso edge weights")
points(allWeights[-rmIndices,2],pch=20,col="blue")
