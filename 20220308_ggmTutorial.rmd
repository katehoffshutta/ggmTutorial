...
The `summary` function tells us our GGM has 155 nodes and 184 edges, along with an attribute for edge weight:
...
```{R}
jankova2015 = SILGGM(x = as.matrix(ocDataSmall), method = "D-S_GL",alpha = 0.05/(155*154), global = T, lambda = 0.322)
```
...
```{R}
inferenceGraph = graph_from_adjacency_matrix(jankova2015$p_precision < 0.05/(155*(155-1)),mode="undirected",diag=F)
...
# Comparison with the adaptive graphical lasso
...
# lasso GGM should be basically the same as stars mat
summary(as.vector(1/2*lassoGGM$P + 1/2*cov2cor(precMat)-0.5*diag(156)))

# lasso GGM should be basically the same as stars mat
summary(as.vector(1/2*adaptiveGGM$P + 1/2*cov2cor(precMat)-0.5*diag(156)))
```

Compare graphs of the stars result, the lasso result, the adaptive lasso result:

```{R}
graph1 = graph_from_adjacency_matrix(-1/2*cov2cor(precMat),weighted=T,mode="plus",diag=F)
graph2 = graph_from_adjacency_matrix(1/2*adaptiveGGM$P,weighted=T,mode="plus",diag=F)
graph3 = graph_from_adjacency_matrix(1/2*lassoGGM$P,weighted=T,mode="plus",diag=F)
```

```{R}
plot(graph1,layout=layout_in_circle)
plot(graph2,layout=layout_in_circle)
plot(graph3,layout=layout_in_circle)
```

```{R}
# The plot we want that will show what's going on is a plot of the edge weights, sorted by lowest to highest, for both the adaptive and regular glasso.
adaptiveEdgeWeights = adaptiveGGM$P[lower.tri(adaptiveGGM$P)]
l1EdgeWeights = lassoGGM$P[lower.tri(lassoGGM$P)]

allWeights = data.frame("adaptive"=adaptiveEdgeWeights,"lasso"=l1EdgeWeights)
allWeights = allWeights[order(allWeights$adaptive),]
# comparing anything to "gold standard" is beyond the scope of this tutorial,that evidence is discussed by Williams and the authors of the original penalties
# remove boring edges where all are zero
rmIndices = which(allWeights$adaptive == 0 & allWeights$lasso == 0)
plot(allWeights[-rmIndices,1],pch=20,col="red")
